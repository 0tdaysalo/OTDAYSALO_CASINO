    ...
    public static class Game
    {
        ...
        public static int make_full_auto_spin_game(int balance, int bet, int n)
        {
            ...
            else
            {
                ...
                int win = Program.pay_out(bet, Program.check_result_spin_char(Program.slot));
                Console.WriteLine($"произошёл {n}-ый спин");
                Program.info_about_spin(win, Program.check_result_spin_char(Program.slot), balance, Program.slot, bet);
                balance = Program.give_money(bet, win, balance);
            }
            return balance;
        }

        //измененная функция make_full_auto_spin_game для тогог чтобы получить итоговый выигрыш
        public static int make_full_auto_spin_game_total_win(int balance, int bet)
        {
            int tot_win = 0;
            if (balance < bet)
            {
        ...
        //играть в казино онлайн в консоле
        ...
        start: { }
            ...
            cont_1: { }
                ...
                int dep = 0;
                if (!Int32.TryParse(Console.ReadLine(), out dep))
                {
            ...
            cont_2: { }
                ...
                if (bet <= 0 || dep <= 0)
                {
                    Console.WriteLine("ТЫ ЧЁ ЕБАНАТ? ПОШЁЛ НАХУЙ ОТСЮДА");
                    Console.WriteLine($"ВВОДИТ ОН БЛЯТЬ {bet} и {dep}, СУКА САМЫЙ УМНЫЙ");
                    return 0;
                }//мат
                ...
                while (true)
                {
                    ...
                    cont_change_dep: { }
                        ...
                        int new_dep = 0;
                        if (!Int32.TryParse(Console.ReadLine(), out new_dep))
                        {
                            Console.WriteLine("некорректный ввод");
                    ...
                    else if (user_input_2 == "ВЫХОД")
                    {
                        ...
                        Console.WriteLine($"ваш баланс после выхода составляет {total_bal} это {((float)total_bal / total_dep) * 100}% от ваших депозитов");
                        return 0;
                    }
                    total_bet += bet;
                    total_win += make_full_auto_spin_game_total_win(dep, bet);
                }
            }
            ...
            else if (user_input_1 == "rule")
            {
                ...
                Console.WriteLine($"тёхтюыюф пидор");//мат
